...
Manggo 原生插件可以扩展翻译、OCR、语音合成和生词本服务。一个插件由清单文件、JavaScript 入口和可选资源组成，使用 Bun 运行，并通过 `manggo.plugin.v1` API 与 Manggo 通信。
...
在 Manggo 的“插件管理”页面安装生成的文件，然后到对应的服务设置页面添加该插件提供的服务。
...
| `kind` | string | 是 | `translation`、`ocr`、`speech` 或 `wordbook`。 |
| `providerId` | string | 否 | 服务提供方 ID；省略时生成 `插件ID#服务ID`。 |
| `displayName` | string | 是 | 用户在服务列表中看到的名称。 |
| `entry` | string | 是 | 入口脚本导出的函数名。 |
...
| `config` | object[] | 否 | 添加或编辑服务时展示的配置表单。 |
...
2. 所有 `services[].entry` 都能从对应入口脚本导出。
3. 清单引用的脚本、图标和资源都已包含在插件包中。
4. 在 Manggo 中安装插件，并实际添加、配置和调用每种服务。
5. 修改内容后更新插件版本并重新打包。
